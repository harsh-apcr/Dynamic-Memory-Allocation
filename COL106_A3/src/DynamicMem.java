...
//   2. Based on binary search trees (Tree package)
...
public abstract class DynamicMem {
    ...
    public BinarySearchTree<MemoryBlock> freeBlk;             // Free blocks dictionary
    public BinarySearchTree<MemoryBlock> allocBlk;           // Allocated blocks dictionary
...
/*        if (type == 2){
            freeBlk = new BinarySearchTree<>(Comparator.comparingInt((MemoryBlock dict) -> dict.key));
            allocBlk = new BinarySearchTree<>(Comparator.comparingInt((MemoryBlock dict) -> dict.key));
            freeBlk.insert(new MemoryBlock(0, size, size));
            // Initially free list has only one block with all the memory
        }           */
        if (type == 3){
            freeBlk = new RBTree<>(Comparator.comparingInt(dict -> dict.key));
            allocBlk = new RBTree<>(Comparator.comparingInt(dict -> dict.key));
            freeBlk.insert(new MemoryBlock(0, size, size));
            // Initially free list has only one block with all the memory
        }
    }
    ...
    // It should return -1 if memory is not available.
